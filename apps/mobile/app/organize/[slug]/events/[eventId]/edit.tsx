...
function isCompeteCategory(entryType: string): boolean {
  return entryType !== 'viewer';
}
...
export default function EventEditScreen() {
  ...
  function sync(detail: OrganizerEventDetailDto) {
    ...
    setAudienceEnabled(Boolean(detail.audience?.enabled));
    setAudiencePrice(String(Math.round((detail.audience?.priceMinor ?? 0) / 100)));
    setAudienceCapacity(String(detail.audience?.capacity || 100));
    setCategoryEdits(
      detail.categories
        .filter((cat) => isCompeteCategory(cat.entryType))
        .map((cat) => ({
          id: cat.id,
          name: cat.name,
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg" edges={['bottom']}>
      <ScrollView contentContainerClassName="gap-3 px-4 pb-10 pt-2">
        ...
        <Text variant="caption">Compete categories only — audience pass is separate.</Text>
        ...
        <Button
          variant="lime"
          onPress={() => router.replace(`/organize/${slug}/events/${eventId}`)}
        >
          Done
        </Button>
      </ScrollView>
